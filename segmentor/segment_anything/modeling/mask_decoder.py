...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        Arguments:
          ...
          cell_nums (torch.Tensor): the number of cells
        ...
        if multimask_output:
            values, indices = torch.max(iou_pred, dim=1)
            iou_pred = values.unsqueeze(1)
            masks = masks[torch.arange(len(iou_pred)), indices].unsqueeze(1)
